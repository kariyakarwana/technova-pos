...
async function handler(
  ...
  let backend = await forward(request, path, body, token);
  ...
  if (backend.status === 401) {
    ...
    if (refresh) {
      ...
      if (refreshed.ok) {
        ...
        rotatedRefresh = refreshed.headers
          .get("set-cookie")
          ?.match(/technova_refresh=([^;]+)/)?.[1];
        ...
        backend = await forward(request, path, body, token);
      }
    }
  }
