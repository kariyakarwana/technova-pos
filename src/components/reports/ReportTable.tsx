...
export default function ReportTable({ title, endpoint, branchScoped = false, dateScoped = true, cashierScoped = false }: { title: string; endpoint: string; branchScoped?: boolean; dateScoped?: boolean; cashierScoped?: boolean }) {
  ...
  useEffect(() => {
    if (cashierScoped) void apiGet<Cashier[]>("/sales/cashiers").then(setCashiers).catch(() => setCashiers([]));
  }, [cashierScoped]);
  useEffect(() => {
    let active = true;
    ...
    if (cashierScoped && cashierId) params.set("cashierId", cashierId);
    ...
    return () => { active = false; };
  }, [endpoint, branchId, branchScoped, cashierId, cashierScoped, dateScoped, from, to]);
  ...
  if (cashierScoped && cashierId) exportParams.set("cashierId", cashierId);
  ...
  return <main className="space-y-6 p-6">
    ...
    {(dateScoped || cashierScoped) && <section className="flex flex-wrap items-end gap-3 rounded-2xl border bg-white p-4">{dateScoped && <><label className="text-xs font-semibold">From<input type="date" value={from} onChange={(event) => setFrom(event.target.value)} className="mt-1 block h-10 rounded-xl border px-3" /></label><label className="text-xs font-semibold">To<input type="date" value={to} min={from} onChange={(event) => setTo(event.target.value)} className="mt-1 block h-10 rounded-xl border px-3" /></label></>}{cashierScoped && <label className="text-xs font-semibold">Cashier<select value={cashierId} onChange={(event) => setCashierId(event.target.value)} className="mt-1 block h-10 min-w-56 rounded-xl border px-3"><option value="">All cashiers</option>{cashiers.map((cashier) => <option key={cashier.id} value={cashier.id}>{cashier.email}</option>)}</select></label>}{branchScoped && <p className="pb-2 text-xs text-slate-500">The branch selector in the top navigation controls this report.</p>}</section>}
