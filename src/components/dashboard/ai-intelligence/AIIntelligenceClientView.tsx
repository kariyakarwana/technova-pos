...
export default function AIIntelligenceClientView() {
  ...
  return <main className="min-h-[calc(100vh-4rem)] w-full min-w-0 bg-[var(--brand-app-bg)] p-4 transition-[width] duration-200 sm:p-6"><div className="w-full min-w-0 space-y-5">
...
function ProductForecastPanel({ product, products, selectedProductId, onSelect }: { product: ProductPrediction | null; products: ProductPrediction[]; selectedProductId: string; onSelect: (id: string) => void }) {
  ...
  return <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1.55fr)_minmax(0,0.8fr)]"><section className="min-w-0 rounded-2xl border border-[var(--brand-stroke)] bg-white p-5 shadow-xs"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-bold text-slate-900">Demand forecast</h2><p className="text-xs text-slate-500">Daily predicted demand using your current POS history</p></div><select value={selectedProductId} onChange={(event) => onSelect(event.target.value)} className="h-9 min-w-0 max-w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold outline-none focus:border-[var(--brand-green)]">{products.map((item) => <option key={item.id} value={item.id}>{item.sku} — {item.name}</option>)}</select></div><div className="mt-5 h-72 min-w-0"><ResponsiveContainer width="100%" height="100%"><LineChart data={product.forecast.daily.slice(0, 90)} margin={{ left: -18, right: 12 }}><CartesianGrid strokeDasharray="3 3" stroke="#e2e8f0" /><XAxis dataKey="date" tick={{ fontSize: 10 }} tickFormatter={(value: string) => value.slice(5)} minTickGap={28} /><YAxis tick={{ fontSize: 10 }} /><Tooltip formatter={(value) => [number.format(Number(value)), "Demand"]} /><Line type="monotone" dataKey="predicted_demand" stroke="#0E9384" strokeWidth={2.5} dot={false} activeDot={{ r: 4 }} /></LineChart></ResponsiveContainer></div></section><section className="min-w-0 rounded-2xl border border-[var(--brand-stroke)] bg-white p-5 shadow-xs"><div className="flex items-center justify-between"><div className="min-w-0"><p className="text-xs font-semibold text-slate-500">{product.sku}</p><h2 className="truncate font-bold text-slate-900">{product.name}</h2></div><span className={`shrink-0 rounded-full border px-2.5 py-1 text-[10px] font-bold uppercase ${statusStyle[plan.status]}`}>{plan.status.replace("_", " ")}</span></div><div className="mt-5 grid grid-cols-2 gap-3">{[["Current stock", plan.current_stock], ["7-day demand", product.forecast.summary.next_7_days_demand], ["30-day demand", product.forecast.summary.next_30_days_demand], ["Order quantity", plan.recommended_order_quantity]].map(([label, value]) => <div key={String(label)} className="min-w-0 rounded-xl bg-slate-50 p-3"><p className="text-[10px] font-semibold text-slate-500">{label}</p><p className="mt-1 text-lg font-bold text-slate-900">{number.format(Number(value))}</p></div>)}</div><div className="mt-4 rounded-xl border border-amber-200 bg-amber-50 p-3"><p className="text-xs font-bold text-amber-800">Confidence: {product.forecast.data_readiness.confidence}</p><p className="mt-1 text-[11px] leading-5 text-amber-700">{product.forecast.data_readiness.recommended_action}</p></div>{plan.recommended_order_quantity > 0 && <Link href={`/purchases/create-order?productId=${product.id}`} className="mt-4 flex h-10 items-center justify-center gap-2 rounded-xl bg-[var(--brand-green)] text-xs font-bold text-white"><PackagePlus className="h-4 w-4" /> Create purchase order</Link>}</section></div>;
}
