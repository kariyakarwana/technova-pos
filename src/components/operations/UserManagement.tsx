...
type User = {
  id: string; email: string; name: string | null; phone: string | null; status: UserStatus;
  lastLoginAt: string | null; roles: { role: Role }[];
  branchAssignments: { isDefault: boolean; branch: Branch }[];
};
const emptyForm = {
  name: "", email: "", phone: "", roleIds: [] as string[], branchIds: [] as string[],
...
export default function UserManagement() {
  ...
  function edit(user: User) {
    ...
    setSelected(user);
    setForm({ name: user.name ?? "", email: user.email, phone: user.phone ?? "", roleIds: user.roles.map(({ role }) => role.id), branchIds, status: user.status, defaultBranchId: user.branchAssignments.find(({ isDefault }) => isDefault)?.branch.id ?? branchIds[0] ?? "" });
  }
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      if (selected) {
        await apiPatch(`/users/${selected.id}/access`, { name: form.name, phone: form.phone, status: form.status, roleIds: form.roleIds, branchIds: form.branchIds, defaultBranchId: form.defaultBranchId || form.branchIds[0] });
        setMessage("Employee access updated. Existing sessions were revoked.");
      } else {
        const created = await apiPost<{ whatsappQueued: boolean }>("/users", { name: form.name, email: form.email, phone: form.phone, roleIds: form.roleIds, branchIds: form.branchIds });
        setMessage(created.whatsappQueued
          ? `Employee account created. The temporary password was emailed to ${form.email}, and a WhatsApp welcome was queued for ${form.phone}.`
          : `Employee account created and emailed to ${form.email}. The WhatsApp message could not be queued; check the WhatsApp configuration and notification template.`);
      }
  ...
  return <main className="space-y-6 bg-[#F8FAFC] p-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[400px_1fr]">
      <form onSubmit={save} className="space-y-4 rounded-2xl border bg-white p-5 shadow-sm">
        <div className="flex items-center justify-between"><div><h2 className="font-bold">{selected ? "Edit employee" : "Create employee"}</h2>{!selected && <p className="mt-1 text-xs text-slate-500">A secure temporary password will be emailed and a WhatsApp welcome will be queued automatically.</p>}</div>{selected && <button type="button" onClick={resetForm} className="text-sm font-semibold text-[#0E9384]">New employee</button>}</div>
        ...
        <label className="block text-xs font-semibold">Phone number *<input required type="tel" inputMode="tel" maxLength={30} pattern="[+0-9][0-9 ()-]{6,29}" placeholder="+94 77 123 4567" value={form.phone} onChange={(event) => setForm({ ...form, phone: event.target.value })} className={input}/></label>
        {!selected && <div className="flex gap-3 rounded-xl border border-teal-100 bg-teal-50 p-3 text-xs text-teal-800"><div className="flex shrink-0 gap-1"><Mail className="h-5 w-5"/><MessageCircle className="h-5 w-5"/></div><p>Email carries the temporary password. WhatsApp confirms the account and sign-in link without exposing the password. If email delivery fails, the account is not created.</p></div>}
        ...
        <button disabled={saving} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#0E9384] px-4 py-2.5 font-semibold text-white disabled:opacity-50"><UserPlus className="h-4 w-4"/>{saving ? "Saving..." : selected ? "Update employee" : "Create and send welcome"}</button>
      </form>
      <section className="min-w-0 overflow-hidden rounded-2xl border bg-white shadow-sm">
        <div className="border-b p-4">
          ...
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            <label className="relative"><Search className="absolute left-3 top-3 h-4 w-4 text-slate-400"/><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Search name, email or phone" className="h-10 w-full rounded-xl border pl-9 pr-3 text-sm"/></label>
