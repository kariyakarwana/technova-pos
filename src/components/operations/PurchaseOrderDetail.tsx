...
export type PurchaseDetail = {
  ...
  supplierResponses: Array<{ id: string; status: string; reviewStatus: string; notes: string | null; respondedAt: string; lines: Array<{ purchaseOrderItemId: string; proposedQuantity: string | number | null; proposedUnitCost: string | number | null; note: string | null }> }>;
  ...
  supplierInvoices: Array<{ id: string; invoiceNumber: string; fileName: string; fileSize: number; uploadedAt: string }>;
};
...
export default function PurchaseOrderDetail({ initial }: { initial: PurchaseDetail }) {
  ...
  async function reviewResponse(responseId: string, approved: boolean) {
    ...
    try {
      const result = await apiPost<{ reviewStatus: string }>(`/supplier-portal/responses/${responseId}/review`, { approved });
      setOrder((current) => ({ ...current, supplierResponses: current.supplierResponses.map((response) => response.id === responseId ? { ...response, reviewStatus: result.reviewStatus } : response) }));
      setMessage(approved ? "Supplier changes approved and applied to the order." : "Supplier changes rejected. The supplier has been notified.");
    } catch (error) { setMessage(error instanceof Error ? error.message : "Unable to review supplier response."); }
    finally { setSaving(false); }
  }
  ...
  return <main className="space-y-6 bg-[#F8FAFC] p-6">
    ...
    {order.supplierResponses.length > 0 && <section className="space-y-3 rounded-2xl border bg-white p-5"><h2 className="font-bold">Supplier responses</h2>{order.supplierResponses.map((response) => <article key={response.id} className="rounded-xl border border-slate-200 p-4"><div className="flex flex-wrap items-center justify-between gap-2"><div><b>{response.status.replaceAll("_", " ")}</b><span className="ml-2 rounded-full bg-slate-100 px-2 py-1 text-xs">{response.reviewStatus.replaceAll("_", " ")}</span></div><time className="text-xs text-slate-500">{new Date(response.respondedAt).toLocaleString()}</time></div>{response.notes && <p className="mt-2 text-sm text-slate-600">{response.notes}</p>}{response.lines.length > 0 && <div className="mt-3 overflow-x-auto"><table className="w-full text-left text-xs"><thead><tr className="bg-slate-50"><th className="p-2">Product</th><th className="p-2">Proposed quantity</th><th className="p-2">Proposed cost</th><th className="p-2">Reason</th></tr></thead><tbody>{response.lines.map((line) => { const item = order.items.find((row) => row.id === line.purchaseOrderItemId); return <tr key={line.purchaseOrderItemId} className="border-t"><td className="p-2">{item?.product.name}</td><td className="p-2">{line.proposedQuantity ?? "No change"}</td><td className="p-2">{line.proposedUnitCost ?? "No change"}</td><td className="p-2">{line.note ?? "—"}</td></tr>; })}</tbody></table></div>}{response.status === "CHANGES_PROPOSED" && response.reviewStatus === "PENDING" && <div className="mt-4 flex gap-2"><button disabled={saving} onClick={() => void reviewResponse(response.id, true)} className="rounded-lg bg-[#0E9384] px-4 py-2 text-xs font-semibold text-white">Approve changes</button><button disabled={saving} onClick={() => void reviewResponse(response.id, false)} className="rounded-lg border border-rose-200 px-4 py-2 text-xs font-semibold text-rose-700">Reject changes</button></div>}</article>)}</section>}
    {(order.supplierShipments.length > 0 || order.supplierInvoices.length > 0) && <section className="grid gap-4 lg:grid-cols-2"><div className="rounded-2xl border bg-white p-5"><h2 className="mb-3 font-bold">Supplier shipments</h2>{order.supplierShipments.length === 0 ? <p className="text-sm text-slate-500">Not dispatched yet.</p> : order.supplierShipments.map((shipment) => <div key={shipment.id} className="mb-2 rounded-xl bg-slate-50 p-3 text-sm"><b>{shipment.status}</b><p>{shipment.carrier ?? "Carrier not recorded"} · {shipment.trackingNumber ?? "No tracking reference"}</p><p className="text-xs text-slate-500">{shipment.dispatchedAt ? new Date(shipment.dispatchedAt).toLocaleString() : "Preparing"}</p></div>)}</div><div className="rounded-2xl border bg-white p-5"><h2 className="mb-3 font-bold">Supplier invoices</h2>{order.supplierInvoices.length === 0 ? <p className="text-sm text-slate-500">No invoice uploaded.</p> : order.supplierInvoices.map((invoice) => <a key={invoice.id} href={`/api/backend/supplier-portal/invoices/${invoice.id}/file`} target="_blank" rel="noreferrer" className="mb-2 flex justify-between rounded-xl bg-slate-50 p-3 text-sm text-[#0E9384]"><b>{invoice.invoiceNumber}</b><span>{invoice.fileName}</span></a>)}</div></section>}
