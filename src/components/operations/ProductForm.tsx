...
export type EditableProduct = {
  ...
  videos?: Array<{ id: string; url: string; originalName?: string | null }>;
};
...
export default function ProductForm({
  ...
  useEffect(() => {
    void Promise.all([
      apiGet<Lookup[]>("/catalog/categories"),
      apiGet<Lookup[]>("/catalog/brands"),
    ])
  ...
  function addImage() {
    ...
    if (imageUrls.length >= 8) {
      setMessage("A product can contain up to eight images.");
      return;
    }
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      if (productId) await apiPatch(`/catalog/products/${productId}`, payload);
      else {
      ...
      if (removeExistingVideo && product?.videos?.[0]) {
        await apiDelete(
          `/catalog/products/${productId}/video/${product.videos[0].id}`,
        );
        setRemoveExistingVideo(false);
      }
      for (const file of imageFiles) {
        await apiUpload(`/catalog/products/${productId}/images`, file);
        uploadedImages += 1;
      }
      setImageFiles([]);
      if (videoFile) {
        await apiUpload(`/catalog/products/${productId}/video`, videoFile);
        setVideoFile(null);
      }
      ...
      if (uploadedImages)
        setImageFiles((current) => current.slice(uploadedImages));
      const reason =
        error instanceof Error ? error.message : "Unable to save product.";
      setMessage(
        productId && !product?.id
          ? `Product details were saved, but media upload did not finish. ${reason} Press Save again to retry.`
          : reason,
      );
    } finally {
      setSaving(false);
  ...
  return (
    <div className="min-h-full overflow-x-hidden bg-[#F8FAFC] p-6">
      ...
      <form onSubmit={save} className="space-y-5">
        <div className="flex items-center justify-between gap-3 rounded-2xl border border-teal-100 bg-teal-50 p-4">
          <div>
            <p className="text-sm font-semibold text-[#025148]">
              Barcode reader / camera
            </p>
            <p className="text-xs text-slate-500">
              Scan a barcode to fill the product barcode field automatically.
            </p>
          </div>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center gap-3 border-b px-5 py-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Identity and catalog classification
              </p>
            </div>
          </div>
          <div className="grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <label className="text-xs font-semibold sm:col-span-2">
              ...
              <textarea
                ...
                className="mt-1 min-h-24 w-full rounded-xl border border-slate-200 p-3 outline-none focus:border-[#0E9384]"
              />
            </label>
          </div>
        </section>
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="border-b px-5 py-4">
            <h2 className="font-bold">Pricing & stock controls</h2>
            <p className="text-xs text-slate-500">
              Values used by purchasing, POS and inventory alerts
            </p>
          </div>
          <div className="grid gap-4 p-5 sm:grid-cols-2 xl:grid-cols-4">
            <label className="text-xs font-semibold">
            ...
            <label className="flex items-center gap-3 rounded-xl border border-teal-100 bg-teal-50 p-4 text-sm font-semibold sm:col-span-2">
              <input
                type="checkbox"
                checked={form.trackSerials}
                onChange={(e) =>
                  setForm({ ...form, trackSerials: e.target.checked })
                }
                className="h-4 w-4 accent-[#0E9384]"
              />
              <span>
                Track each unit by serial number and secure QR
                <span className="mt-1 block text-xs font-normal text-slate-500">
                  Required for one-time warranty activation and serialized
                  traceability.
                </span>
              </span>
            </label>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="flex items-center gap-3 border-b px-5 py-4">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                Add up to eight hosted image URLs. The first image is used in
                POS and catalog cards.
              </p>
            </div>
          </div>
          <div className="p-5">
            <div className="flex max-w-3xl gap-2">
              <input
                ...
                placeholder="https://cdn.example.com/products/item.jpg"
                className={input.replace("mt-1 ", "")}
              />
              <button
                type="button"
                onClick={addImage}
                className="inline-flex items-center gap-2 rounded-xl bg-[#025148] px-4 py-2 text-sm font-semibold text-white"
              >
            ...
            <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
              {imageUrls.map((url, index) => (
                <div
                  key={`${url}-${index}`}
                  className="group relative overflow-hidden rounded-2xl border bg-slate-50"
                >
              ...
              {imageUrls.length === 0 && (
                <div className="col-span-full rounded-2xl border border-dashed p-8 text-center text-sm text-slate-500">
                  <ImagePlus className="mx-auto mb-2 h-7 w-7 text-slate-300" />
        ...
        <ProductMediaUploadFields
          ...
          existingVideo={product?.videos?.[0]}
          removeExistingVideo={removeExistingVideo}
          onRemoveExistingVideoChange={setRemoveExistingVideo}
        ...
        <div className="flex justify-end gap-3">
          ...
          <button
            disabled={saving}
            className="rounded-xl bg-[#0E9384] px-6 py-2.5 font-semibold text-white disabled:opacity-50"
          >
