...
export default function ProductForm({ product }: { product?: EditableProduct }) {
  ...
  return <main className="min-h-screen bg-[#F8FAFC] p-6">
    ...
    <form onSubmit={save} className="space-y-5">
      <div className="flex items-center justify-between gap-3 rounded-2xl border border-teal-100 bg-teal-50 p-4"><div><p className="text-sm font-semibold text-[#025148]">Barcode reader / camera</p><p className="text-xs text-slate-500">Scan a barcode to fill the product barcode field automatically.</p></div><BarcodeScannerButton onScan={(barcode) => setForm((current) => ({ ...current, barcode }))} /></div>
