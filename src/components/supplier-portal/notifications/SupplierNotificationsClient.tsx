...
export default function SupplierNotificationsClient({
  initial,
  loadError,
}: {
  initial: SupplierNotification[];
  loadError?: string;
}) {
  const [items, setItems] = useState(initial);
  if (loadError && initial.length === 0)
    return (
      ...
      <section className="space-y-3">
        {items.length === 0 ? (
          ...
          items.map((item) => (
            <article
              key={item.id}
              className={`rounded-2xl border p-5 ${item.readAt ? "bg-white" : "border-teal-200 bg-teal-50"}`}
            >
              <div className="flex gap-3">
                <BellRing className="mt-0.5 h-5 w-5 text-[#0E9384]" />
                  ...
                  <div className="mt-3 flex gap-2">
                    {item.actionUrl && (
                      <Link
                        ...
                        className="rounded-lg bg-[#0E9384] px-3 py-2 text-xs font-semibold text-white"
                      >
                        Open order
                      </Link>
                    )}
